...
"""Run visual diffusion policy data-scale and input-ablation experiments."""
...
def _stages(requested: list[str]) -> set[str]:
    if "all" in requested:
        return {"collect", "filter", "inspect", "preview", "train", "eval", "plot"}
    return set(requested)


def _conditions(requested: list[str]) -> list[str]:
    if "all" in requested:
        return ["rgb_lowdim", "image_only"]
    seen = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run BallBasket visual diffusion experiments.")
    ...
    parser.add_argument("--image_sizes", type=int, nargs="+", default=[96], help="Rendered RGB image sizes to compare.")
    parser.add_argument("--seeds", type=int, nargs="+", default=[0], help="Training/evaluation seeds.")
    parser.add_argument(
        "--conditions",
        nargs="+",
        default=["rgb_lowdim"],
        choices=["rgb_lowdim", "image_only", "all"],
        help="Input conditions to train. rgb_lowdim is RGB plus low-dimensional state.",
    )
    parser.add_argument(
        "--stages",
        nargs="+",
        default=["all"],
        choices=["all", "collect", "filter", "inspect", "preview", "train", "eval", "plot"],
    ...
    parser.add_argument("--preview_episodes", type=int, default=3, help="Preview this many episodes per filtered dataset.")
    parser.add_argument("--preview_gif", action="store_true", default=False, help="Save GIFs during preview.")
    ...
    parser.add_argument("--cache_images", action="store_true", default=False, help="Load images into RAM while training.")
    parser.add_argument("--image_feature_dim", type=int, default=128, help="Visual encoder feature dimension.")
    ...
    parser.add_argument("--num_episodes_eval", type=int, default=5, help="Evaluation episodes.")
    parser.add_argument("--video_eval", action="store_true", default=False, help="Record eval videos for each run.")
    parser.add_argument("--dataset_root", type=str, default="datasets/ball_basket_visual/experiments")
    parser.add_argument("--run_root", type=str, default="runs/visual_diffusion")
    ...
    if any(image_size <= 0 for image_size in args.image_sizes):
        raise ValueError("image_sizes must be positive.")

    experiment_name = args.name or _timestamp_name()
    dataset_root = Path(args.dataset_root) / experiment_name
    ...
    print(f"[INFO]: image_sizes={args.image_sizes}")
    ...
    for image_size in args.image_sizes:
        dataset_dir = dataset_root / f"image_{image_size}"
        for demo_count in args.demo_counts:
            ...
            print(f"\n[INFO]: === image_size={image_size} demo_count={demo_count} ===")

            if "collect" in stages:
                collect_cmd = (
                    _python_script("collect_visual_demos.py")
                    + [
                        "--task",
                        args.task,
                        "--num_demos",
            ...
            if "preview" in stages:
                preview_dir = dataset_dir / f"filtered_{demo_count}_preview"
                preview_cmd = [
                    *_python_script("preview_visual_dataset.py"),
                    str(filtered_dataset),
                    "--output_dir",
                    str(preview_dir),
                    "--num_episodes",
                    str(args.preview_episodes),
                ]
                if args.preview_gif:
                    preview_cmd.append("--gif")
                _maybe_run(preview_cmd, preview_dir / "preview_manifest.json", args.skip_existing, args.dry_run)

            for condition in conditions:
                for seed in args.seeds:
                    run_dir = run_root / f"image_{image_size}" / condition / f"demos_{demo_count}" / f"seed_{seed}"
                    checkpoint = run_dir / "policy.pt"
                    best_checkpoint = run_dir / "best.pt"
                    eval_metrics = run_dir / f"eval_{args.num_episodes_eval}_episodes.json"
                    run_config = {
                        **experiment_config,
                        "demo_count": demo_count,
                        "image_size": image_size,
                        "condition": condition,
                        "use_lowdim_obs": condition == "rgb_lowdim",
                        "seed": seed,
                    ...
                    if "train" in stages:
                        train_cmd = [
                            *_python_script("train_visual_diffusion.py"),
                            "--dataset",
                            str(filtered_dataset),
                            "--epochs",
                            str(args.epochs),
                        ...
                        if args.cache_images:
                            train_cmd.append("--cache_images")
                        if condition == "image_only":
                            train_cmd.append("--image_only")
                        _maybe_run(train_cmd, checkpoint, args.skip_existing, args.dry_run)

                    if "eval" in stages:
                        eval_cmd = [
                            *_python_script("eval_visual_diffusion.py"),
                            "--task",
                            args.task,
                            "--checkpoint",
                            str(best_checkpoint),
                        ...
                        if args.video_eval:
                            eval_cmd += ["--video", "--video_dir", str(run_dir / "videos")]
                        _maybe_run(eval_cmd, eval_metrics, args.skip_existing, args.dry_run)

    if "plot" in stages:
        _run(
            _python_script("plot_visual_results.py")
